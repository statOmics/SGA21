...
## Parameter estimation and empirical Bayes
 ...
 $$
 Here, the posterior distribution $p(\theta | \mathbf{Y})$ is calculated based on the data likelihood $p(\mathbf{Y} | \theta)$, prior distribution $p(\theta)$ and the 'marginal likelihood' $\int_{\theta \in \Theta} p(\mathbf{Y} | \theta) p(\theta) d\theta$, where $\Theta$ denotes the parameter space of $\theta$.
 - In empirical Bayes, we basically take a semi-Bayesian approach to parameter estimation. Indeed, what makes it empirical is that we are using the data to estimate the prior distribution, unlike in a fully Bayesian analysis where the prior distribution must be specified independently of the data.
